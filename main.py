...
PRICE_PER_TRY = 10  # Цена за одну примерку в рублях
UPLOAD_DIR = "uploads"
...
# Настройки Payeer
PAYEER_WALLET = "P1043320311"
PAYEER_PAYMENT_URL = f"https://payeer.com/merchant/?m_shop=1043320311&m_orderid={{order_id}}&m_amount={{amount}}&m_curr=RUB&m_desc={{description}}"
...
async def send_welcome(user_id: int, username: str, full_name: str):
    ...
    try:
        ...
        await bot.send_message(
            ...
            "⚠️‼️ <b>Важная информация - перед использованием Примерочной УБЕДИТЕСЬ:</b>\n\n"
            "1️⃣ <b>У Вас в телегамм стоит ИМЯ ПОЛЬЗОВАТЕЛЯ (посмотрите в Настройках - если его нет то придумайте и напишите на англ.)</b>\n"
            "2️⃣ <b>Вы не заблокировали бота и можете получать от него сообщения.</b>\n\n"
            "⚠️<b>Иначе не придёт результат примерки </b>\n\n"  
...
async def show_payment_options(user: types.User):
    ...
    try:
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text="💰 Оплатить Payeer", 
                    callback_data="pay_payeer"
                )
            ],
            [
                InlineKeyboardButton(
        ...
        payment_text = (
            ...
            "Первая примерка была демонстрационной, последующие примерки стоят 10 рублей за примерку.\n"
            ...
            f"- 30 руб = 3 примерки\n"
            "и так далее...\n\n"
            "1️⃣ Выберите удобный способ оплаты (картой,через СБП, по QR-коду или Payeer)\n\n"
            "2️⃣ Введите сумму\n\n"
            ...
            "‼️<b>Ничего не меняйте в сообщении</b>\n\n"
            "💳При оплате картой в поле <b>Никнейм</b> укажите <b>Ваше имя телеграмм</b>\n\n"
            "Кнопку <b>Войти</b> нажимать не надо, переходите в поле <b>Сообщение</b> вставляете скопированныне комментарии к платежу и оплачиваете\n\n"
            "🔁 После оплаты — нажмите кнопку <b>«Я оплатил»</b>\n\n"
            "❓Свой баланс Вы можете отслеживать по кнопке <b>'Мой баланс'</b>"
        )
...
@dp.callback_query(F.data == "pay_payeer")      
async def pay_payeer_handler(callback_query: types.CallbackQuery):
    """Обработчик кнопки оплаты через Payeer"""
    user = callback_query.from_user
    payeer_instructions = (
        "📲 <b>Как оплатить через Payeer:</b>\n\n"
        "1️⃣ Перейдите на сайт Payeer или откройте приложение\n"
        "2️⃣ Введите номер кошелька получателя:\n\n"
        f"💳 <code>{PAYEER_WALLET}</code>\n\n"
        "3️⃣ Введите сумму (например, 10, 20, 30 рублей)\n\n"
        "4️⃣ <b>ОБЯЗАТЕЛЬНО укажите в комментарии:</b>\n\n"
        f"💬 <code>ОПЛАТА ЗА ПРИМЕРКИ от @{user.username or 'ваш_ник'}</code>\n\n"
        "🔹 <i>Нажмите, чтобы скопировать</i>\n\n"
        "<b>🤷‍♂️Иначе не будет понятно кому начислять баланс.</b>\n\n"
        "5️⃣ Подтвердите платёж\n\n"
        "🔁 После оплаты нажмите кнопку <b>«Я оплатил»</b>"
    )
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(
                text="💳 Оплатить через Payeer",
                url=f"https://payeer.com/merchant/?m_shop=1043320311&m_orderid={user.id}&m_amount=10&m_curr=RUB&m_desc=ОПЛАТА ЗА ПРИМЕРКИ от @{user.username or 'ваш_ник'}"
            )
        ],
        [
            InlineKeyboardButton(
...
5️⃣ Введите сумму (например, 10, 20, 30 рублей)
