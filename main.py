...
class SupabaseAPI:
    def __init__(self):
        ...
        self.sent_results = set()      # Множество для отслеживания отправленных результатов
    ...
    def mark_result_as_sent(self, user_id: int):
        """Помечает результат как отправленный"""
        self.sent_results.add(user_id)

    def is_result_sent(self, user_id: int) -> bool:
        """Проверяет, был ли результат уже отправлен"""
        return user_id in self.sent_results

    def clear_sent_result(self, user_id: int):
        """Удаляет отметку о отправленном результате"""
        self.sent_results.discard(user_id)
...
async def handle_photo(message: types.Message):
    ...
    if user_row and user_row.get("ready"):
        ...
        await supabase_api.reset_flags(user_id)
        supabase_api.clear_sent_result(user_id)  # Очищаем отметку о отправке
    ...
    while True:
        try:
            ...
            for user_id_str in os.listdir(UPLOAD_DIR):
                ...
                # Проверяем, не был ли результат уже отправлен
                if supabase_api.is_result_sent(user_id):
                ...
                if not result_files:
                    for ext in SUPPORTED_EXTENSIONS:
                        try:
                            result_supabase_path = f"{user_id}/result{ext}"
                            ...
                            logger.info(f"✅ Скачан result{ext} из Supabase для пользователя {user_id}")
                ...
                if result_files:
                    ...
                    try:
                        ...
                        supabase_api.mark_result_as_sent(user_id)
