...
async def show_payment_options(user: types.User):
    ...
    try:
        ...
        payment_text = (
            ...
            "👇👇👇👇👇👇👇👇👇👇\n"
            f"<code>ОПЛАТА ЗА ПРИМЕРКИ от от {full_name}@{user.username} личный код {user.id}</code>\n\n"
			" ⬆️    ⬆    ️⬆️"
...
async def pay_qr_handler(callback_query: types.CallbackQuery):
    ...
    caption = (
        ...
        f"<code>ОПЛАТА ЗА ПРИМЕРКИ от {full_name}@{user.username} личный код {user.id}</code>\n\n"
		" ⬆️    ⬆    ️⬆️\n"
        "🔹 <i>Нажмите, чтобы скопировать</i>\n\n"
		"<b>🤷‍♂️Иначе не будет понятно кому начислять баланс.</b>\n\n"
...
💬 <code>ОПЛАТА ЗА ПРИМЕРКИ от от {full_name}@{user.username} личный код {user.id}</code>
