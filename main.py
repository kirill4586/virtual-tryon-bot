...
async def show_payment_options(user: types.User):
    ...
    try:
        ...
        payment_text = (
            ...
            "️⃣  Нажмите на кнопку 'Пополнить баланс'\n"
            "2️⃣  Указываете любую сумму не менее 30 руб (сколько хотите примерок)\n"
			f"⚠️‼️ <b>ВНИМАНИЕ!</b> В поле для собщений, которое находится под оплатой обязательно укажите:\n"
              "ОПЛАТА ЗА ПРИМЕРКИ ОТ @Vladimir5446 (ID: 973853935)\n"
              "Иначе не будет понятно кому начислять баланс.\n"

            "Как это сделать:\n"
			"Писать Вам ничего не нужно просто нажмите на это сообщение:\n"
            f"<code>Оплата за примерки от @{user.username} (ID: {user.id})</code>\n\n"
           
           "Вы увидите, что оно скопировано и вствьте в  поле Для  на странице оплаты.\n"

		   "Не изменяйте это сообщение, иначе оплата не пройдёт!"\n\n"
        
        
            "3️⃣ Выбираете удобный способ оплаты (Карта или СБП)\n\n"
			"4️⃣ В поле e-mail - укажите любую почту, можете свою, можете придумать(не имеет значения)\n"
           ...
           "❓Свой баланс Вы можете отслеживать по кнокпке "Узнать баланс"\n"
