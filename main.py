...
async def handle_payment_confirmation(callback_query: types.CallbackQuery):
    ...
    try:
        ...
        # Уведомим админа о попытке подтверждения
        if ADMIN_CHAT_ID:
            await bot.send_message(
                ADMIN_CHAT_ID,
...
@dp.message_handler(lambda message: message.text and message.reply_to_message and "укажите одно из следующего" in message.reply_to_message.text)
async def handle_payment_fio(message: types.Message):
    """Обработчик текста (ФИО)"""
    try:
        fio = message.text
...
async def handle_payment_screenshot(message: types.Message):
    ...
    try:
        await message.reply("✅ Спасибо, скриншот получен. Ожидайте подтверждения.")
