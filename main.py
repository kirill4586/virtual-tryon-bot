...
async def show_payment_options(user: types.User):
    ...
    try:
        ...
        payment_text = (
            ...
            "💥<b>ВСЁ!!!</b>💥\n\n"
            "⚠️‼️ <b>ВНИМАНИЕ!</b> При оплате в поле для сообщений, которое находится под оплатой обязательно укажите следующее:</b>\n\n"
            "👇👇👇👇👇👇👇👇👇👇\n"
            f"<b><code>ОПЛАТА ЗА ПРИМЕРКИ от @{user.username}</code></b>\n\n"
            "Просто нажмите на это сообщение, оно скопируется и вставьте его в поле для сообщений, под суммой оплаты</b>\n\n"
            "🤷‍♂️Иначе не будет понятно кому начислять баланс.\n"
