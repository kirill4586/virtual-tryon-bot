...
async def send_welcome(user_id: int, username: str, full_name: str):
    ...
    try:
        ...
        await bot.send_message(
            ...
            "⚠️‼️ <b>Важно:</b>\n\n"
            "<b>Ваш телеграмм должен принимать сообщения (не стоять ограничений)</b>\n\n"
...
async def show_payment_options(user: types.User):
    ...
    try:
        ...
        full_name = getattr(user, "full_name", "Без имени")
        username = user.username if user.username else "без_username"
        user_id = user.id

        payment_text = (
            "🚫 У вас закончились бесплатные примерки.\n\n"
            "❤️ Спасибо, что воспользовались нашей Виртуальной примерочной! 🥰\n"
            "Первая примерка была демонстрационной, последующие стоят 10 рублей за примерку.\n"
            "Сумма символическая, но она поможет вам стать стильными, модными и красивыми!\n"
            "👗👔🩳👙👠👞👒👟🧢🧤👛👜\n\n"
            ...
            "👇👇👇👇👇👇👇👇👇👇\n"
            f"<code>ОПЛАТА ЗА ПРИМЕРКИ от {full_name}@{username} личный код {user_id}</code>\n\n"
            "⬆️    ⬆    ️⬆️\n"
            "🔹 <i>Нажмите, чтобы скопировать</i>\n\n"
            "<b>🤷‍♂️ Иначе не будет понятно, кому начислять баланс.</b>\n\n"
            "‼️ <b>Ничего не меняйте в сообщении</b>\n\n"
            "💳 При оплате картой в поле <b>Никнейм</b> укажите <b>ваше имя в Telegram</b>\n\n"
            "Кнопку <b>Войти</b> нажимать не надо — переходите в поле <b>Сообщение</b>, вставьте комментарий и оплатите.\n\n"
        ...
        if ADMIN_CHAT_ID:
            try:
                await bot.send_message(
                    ...
                    f"ℹ️ Сообщение для ЮMoney: 'ОПЛАТА ЗА ПРИМЕРКИ от {full_name}@{user.username} личный код {user.id}'"
                )
            except Exception as e:
                logger.error(f"Error sending admin payment notification: {e}")
...
💬 <code>ОПЛАТА ЗА ПРИМЕРКИ от {full_name}@{user.username} личный код {user.id}</code>
