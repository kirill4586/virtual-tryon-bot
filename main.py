...
payment_note = f"<b>ОПЛАТА ЗА ПРИМЕРКИ от @{user.username}</b>"
...
payment_text = (
    ...
    "💥<b>ВСЁ!!!</b>💥\n\n"
    "⚠️‼️ <b>ВНИМАНИЕ!</b> При оплате в поле для сообщений укажите следующее:\n\n"
    "👇👇👇👇👇👇👇👇👇👇\n\n"
    f"{payment_note}\n\n"
    "Просто нажмите на это сообщение, оно скопируется и вставьте его в поле для сообщений, под суммой оплаты\n\n"
    "🤷‍♂️Иначе не будет понятно кому начислять баланс.\n"
    "‼️<b>Ничего не меняйте в сообщении‼️</b>\n\n"
    "❓Свой баланс Вы можете отслеживать по кнопке 'Мой баланс'"
)

await bot.send_message(user.id, payment_text, parse_mode=ParseMode.HTML)
...
if __name__ == "__main__":
    try:
        ...
        loop.close()
        logger.info("Bot successfully shut down")import os
import logging
import asyncio
import aiohttp
import shutil
